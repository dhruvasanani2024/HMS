...
def verify_login_otp(request):
    if request.method == 'POST':
        ...
        if otp == session_otp:
            ...
            return render(request, 'otp_verify.html', {'submit_url': '/verify-login-otp/'})
    return redirect('login')
...
def verify_registration_otp(request):
    if request.method == 'POST':
        ...
        if otp == session_otp:
            ...
            messages.error(request, 'Invalid OTP. Please try again.')
            return render(request, 'otp_verify.html', {'submit_url': '/verify-registration-otp/'})
    return redirect('login')
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            student = Students_data.objects.filter(user=user).first()
            if student and student.phone:
                phone_number = "+91" + student.phone if not student.phone.startswith("+") else student.phone
                try:
                    client = Client(settings.TWILIO_ACCOUNT_SID, settings.TWILIO_AUTH_TOKEN)
                    message = client.messages.create(
                        body=f"Your HMS Login OTP is: {otp}. Valid for 5 minutes.",
...
def RegisterNewStudent(request):
    if request.method == 'POST':
        ...
        # Send OTP via Twilio
        phone_number = "+91" + phone if not phone.startswith("+") else phone
        try:
            client = Client(settings.TWILIO_ACCOUNT_SID, settings.TWILIO_AUTH_TOKEN)
            message = client.messages.create(
                body=f"Your HMS Registration OTP is: {otp}. Valid for 5 minutes.",
                from_=settings.TWILIO_PHONE_NUMBER,
                to=phone_number
            )
        except Exception as e:
